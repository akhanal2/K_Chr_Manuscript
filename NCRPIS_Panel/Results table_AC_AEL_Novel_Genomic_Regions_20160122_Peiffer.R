...
#$K_chr Approach$
  ...
  #-----Flowering Time-------
dataPath="Conduct_K_chr_Peiffer/"
...
#$Traditional Q+K$
  ...
  #-----Flowering Time-------
dataPath="Pheno+Geno_Data_Peiffer/"
traitnames <- c("ALL_DTA_BLUE","ALL_EHT_BLUE", "ALL_PHT_BLUE")
total.count.1<-0
total.count.2<-0
for (i in traitnames){
...
#-----Flowering Time-------
dataPath.1="Conduct_K_chr_Peiffer/"
...
for (i in traitnames){
  ...
  if(count!= 0){
    ...
    write.table(true.novel.genom.region, paste("Novel.genomic.regions.detected.for", i,".txt", sep = ""), quote = FALSE, 
                sep = "\t", row.names = FALSE,col.names = TRUE)
    
    #Export "cum.novel.genom.region" to a tab delmited text file
    write.table(cum.novel.genom.region, paste("Number.of.Novel.genomic.regions.detected.for", i,".txt", sep = ""), quote = FALSE, 
                sep = "\t", row.names = FALSE,col.names = TRUE)
  }
...
for (i in traitnames){
  ...
  results.Kchr <-read.csv(paste(dataPath.1,"GAPIT.",i,".GWAS.Results_Kchr.csv",sep=""),head=TRUE)
  results.class <-read.csv(paste(dataPath.2,"GAPIT.",i,".GWAS.Results.csv",sep=""),head=TRUE)
  sig.pvalue.Kchr <-results.Kchr[which(results.Kchr[,11]<=0.10),]
  sig.pvalue.class <-results.class[which(results.class[,10]<=0.10),]
  novel.genom.chrom <- sig.pvalue.Kchr$Chromosome
