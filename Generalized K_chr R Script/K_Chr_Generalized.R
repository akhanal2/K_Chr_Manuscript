...
#############    
  for(i in myfile.from:myfile.to){ #For loop through each chromosome
    ...
    print(paste("------------------------------- Chromosome ", i, ": Just finished calculating separate kinship matrices, now performing association analysis on this chromosome... -----------------------------  ", sep = ""))
    ...
    myGAPIT <- GAPIT(
      ...
      group.to=mygroup.to,			#Upper bound for number of group
      #group.by=1,
